...
void vecquant123matmul_mixed(
  ...
  const at::cuda::OptionalCUDAGuard device_guard(device_of(vec));
  vecquant123matmul_mixed_cuda(vec, mat, mul, scales, zeros, g_idx, bitw, infeatures, group_size);
}
...
{
  const at::cuda::OptionalCUDAGuard device_guard(device_of(vec));
  vecquant234matmul_mixed_cuda(vec, mat, mul, scales, zeros, g_idx, bitw, infeatures, group_size);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("vecquant123matmul_mixed", &vecquant123matmul_mixed, "Vector 2-bit Mixed-Precision Quantized Matrix Multiplication (CUDA) (desc_act)");
  m.def("vecquant234matmul_mixed", &vecquant234matmul_mixed, "Vector 3-bit Mixed-Precision Quantized Matrix Multiplication (CUDA) (desc_act)");
