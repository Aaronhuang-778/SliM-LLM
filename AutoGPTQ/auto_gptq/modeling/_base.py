...
"""
        ...
        for i in range(len(layers)):
            ...
            subset = {}
            for names in inside_layer_modules:
                subset.update({n: full[n] for n in names if n in full})
            
            for name in subset:
                logger.info(f"Quantizing {name} in layer {i + 1}/{len(layers)}...")
                # scale zero g_idx from outside
                scale = in_scales['model.layers.%d.%s' % (i, name)]
                zero = in_zeros['model.layers.%d.%s' % (i, name)]
                g_idx = in_g_idxes['model.layers.%d.%s' % (i, name)]
                block_bit = in_bits['model.layers.%d.%s' % (i, name)]
                quantizers[f"{self.layers_block_name}.{i}.{name}"] = (
                    None,
                    move_to_device(scale, CPU if force_layer_back_to_cpu else cur_layer_device),
                    move_to_device(zero, CPU if force_layer_back_to_cpu else cur_layer_device),
                    move_to_device(g_idx, CPU if force_layer_back_to_cpu else cur_layer_device),
                    move_to_device(block_bit, CPU if force_layer_back_to_cpu else cur_layer_device),
                )
                qweight_height = 0
                for bit in block_bit.to(CPU):
                    bit = bit.item()
                    if bit == 1:
                        bit = 2
                    qweight_height += self.quantize_config.group_size // 32 * bit
                qweight_heights[f"{self.layers_block_name}.{i}.{name}"] = qweight_height
        self.quantize_config.qweight_heights = qweight_heights
        ...
        pack_model(
            ...
            group_size=self.quantize_config.group_size,
            qweight_heights=self.quantize_config.qweight_heights,
        ...
        if not use_qigen:
            ...
            with ContextManagers(init_contexts):
                ...
                make_quant(
                    ...
                    quantize_config.qweight_heights,
                    use_triton=use_triton,
